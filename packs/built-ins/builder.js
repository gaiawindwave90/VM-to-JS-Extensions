const SOURCE = "https://gaiawindwave90.github.io/VM-to-JS-Extensions/";
const SOURCE_URL = `${SOURCE}Gallery%20Files/Extension-Keys.json`;

const creator = {
  name: "Former VM Collection",
  description: "A collection of TurboWarp/PotentiaMod extension originaly made from VM built-ins.",
  upstream: SOURCE,
};
...
const build = async ({ existingPack }) => {
  try {
    ...
    if (!data.extensions || Array.isArray(data.extensions)) {
      throw new Error("Expected the legacy Extension-Keys.json format");
    }
    return convertLegacy(data, async (fileName) => {
      const scriptResponse = await fetch(`${SOURCE}code/${fileName}`);
    ...
    if (existingPack?.information && Array.isArray(existingPack.extensions)) {
      console.warn(
        `Could not refresh pack; using generated copy: ${error}`
      );
      return existingPack;
    }
    throw error;
